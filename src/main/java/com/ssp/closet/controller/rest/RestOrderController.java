...
public class RestOrderController {
	...
	public Delivery getOrder(@PathVariable("orderId") int orderId, HttpServletResponse response)
		...
		Delivery order = null;
		//order = closet.getOrderDetail(orderId);
		if (order == null) {
	...
	public List<Delivery> getOrdersByUsername(@PathVariable("username") String username, HttpServletResponse response)
			throws IOException {
		System.out.println("/rest/order/{username} request accepted: {username} = " + username);
		List<Delivery> orderList = null;
		//orderList = closet.getOrdersByUsername(username);
		if (orderList == null) {
			response.sendError(HttpServletResponse.SC_NOT_FOUND);
			return null;
		}
